...
function EditWorkflows() {
    ...
    const workflowColumnsRef = useRef({});
    ...
    // Calculate positions for limit cells
    useEffect(() => {
        const updateCellPositions = () => {
            Object.keys(workflowColumnsRef.current).forEach(key => {
                const columnElement = workflowColumnsRef.current[key];
                if (columnElement) {
                    const cellElement = document.querySelector(`.efb_cell.workflow-area-cell[data-area="${key}"]`);
                    if (cellElement) {
                        const wrapperElement = columnElement.closest('.workflow-columns-wrapper');
                        if (wrapperElement) {
                            const columnRect = columnElement.getBoundingClientRect();
                            const wrapperRect = wrapperElement.getBoundingClientRect();
                            const stagesContainer = columnElement.querySelector('.workflow-stages-container');

                            if (stagesContainer) {
                                const stagesRect = stagesContainer.getBoundingClientRect();
                                const left = columnRect.left - wrapperRect.left;
                                const width = columnRect.width;
                                const top = stagesRect.bottom - wrapperRect.top + 8; // 8px gap
            ...
            // Position Portfolio Lane cell
            const portfolioLaneColumn = document.querySelector('.portfolio-lane-column');
            const portfolioLaneCell = document.querySelector('.portfolio-lane-cell');
            if (portfolioLaneColumn && portfolioLaneCell) {
                const wrapperElement = portfolioLaneColumn.closest('.workflow-columns-wrapper');
                if (wrapperElement) {
                    const columnRect = portfolioLaneColumn.getBoundingClientRect();
                    const wrapperRect = wrapperElement.getBoundingClientRect();
                    const stagesContainer = portfolioLaneColumn.querySelector('.workflow-stages-container');

                    if (stagesContainer) {
                        const stagesRect = stagesContainer.getBoundingClientRect();
                        const top = stagesRect.bottom - wrapperRect.top + 8; // 8px gap
                        portfolioLaneCell.style.top = `${top}px`;
                    }
                }
            }
        };
        ...
        const timeoutId = setTimeout(updateCellPositions, 100);
                    ...
                    {workflows.map((workflow) => (
                        <div key={workflow.id} className="workflow-card">
                            ...
                            {workflow.swimlanes.map((swimlane) => {
                                ...
                                // Get all areas for limit cells positioning
                                const allAreas = Object.keys(stagesByArea);
                                const portfolioLaneWidth = 180; // Portfolio Lane column width

                                return (
                                    <div key={swimlane.id} className="workflow-swimlane">
                                        <div className="swimlane-label">{swimlane.name}</div>
                                        <div className="workflow-columns-wrapper">
                                            {/* Portfolio Lane Column */}
                                            <div className="workflow-column portfolio-lane-column">
                                                <div className="portfolio-lane-header"></div>
                                                <div className="workflow-stages-container">
                                                    <div className="workflow-stages-row">
                                                        <div className="portfolio-lane-box">
                                                            <div className="portfolio-lane-content">
                                                                <span className="portfolio-lane-title">{swimlane.name}</span>
                                                                <div className="portfolio-lane-icons">
                                                                    <button className="portfolio-lane-icon-btn" type="button" title="Global">
                                                                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                                            ...
                                                                            <path d="M1.33333 8H14.6667M8 1.33333C9.66733 4.33333 10.6667 6.16667 10.6667 8C10.6667 9.83333 9.66733 11.6667 8 14.6667C6.33267 11.6667 5.33333 9.83333 5.33333 8C5.33333 6.16667 6.33267 4.33333 8 1.33333Z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                                                                        </svg>
                                                                    </button>
                                                                    <button className="portfolio-lane-icon-btn" type="button" title="Settings">
                                                                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                                    ...
                                                                    <button className="portfolio-lane-icon-btn" type="button" title="Time tracking">
                                                                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                ...
                                                {Object.entries(stagesByArea).map(([area, rows]) => {
                                                    ...
                                                    return (
                                                        <div
                                                            ...
                                                            ref={(el) => {
                                                                if (el) {
                                                                    workflowColumnsRef.current[area] = el;
                                                                } else {
                                                                    delete workflowColumnsRef.current[area];
                                                                }
                                                            }}
                                                        >
                                                            <div
                                            ...
                                            {/* Limit Cells Container (efb_cells) */}
                                            <div className="efb_cells">
                                                {/* Portfolio Lane Limit Cell */}
                                                <div className="efb_cell portfolio-lane-cell">
                                                    <div className="workflow-stage-placeholder">
                                                        <span>Limit: 0</span>
                                                    </div>
                                                </div>
                                                {/* Workflow Area Limit Cells */}
                                                {allAreas.map((area) => (
                                                    <div
                                                        key={area}
                                                        className="efb_cell workflow-area-cell"
                                                        data-area={area}
                                                    >
                                                        <div className="workflow-stage-placeholder">
                                                            <span>Limit: 0</span>
